...
class Meter():
    def __init__(self, clf_task=None):
        ...
        self.loss_ = []
        self.pt_names = []
    def add(self, spectrum, label, channel_name, start_end, intensity, waveform, output, loss=0, pt_name = np.array(["None"])):
        self.outputs.append(output)
        self.label.append(label)
        self.channel_name.append(channel_name)
        self.start_end.append(start_end.numpy())
        #self.loss_.append(loss)
        self.pt_names.append(pt_name)
    ...
    def f1(self):
        outputs = merge_listinfo(self.outputs)
        labels = merge_listinfo(self.label)
        f1_s = f1_score(labels.squeeze(), (outputs>0.5).squeeze())
        recall_s = recall_score(labels.squeeze(), (outputs>0.5).squeeze())
        precision_s = precision_score(labels.squeeze(), (outputs>0.5).squeeze())
        balance_acc = balanced_accuracy_score(labels.squeeze(), (outputs>0.5).squeeze())
        return f1_s, recall_s, precision_s, balance_acc
    ...
    def dump_csv(self, filename):
        ...
        channel_name = merge_listinfo(self.channel_name)
        start_end = merge_listinfo(self.start_end)
        pt_name = merge_listinfo(self.pt_names)
        
        if len(outputs) == 0:
            print("no data for this patient")
            return 
    ...
    def dump_prediction(self, filename):
        ...
        channel_name = merge_listinfo(self.channel_name)
        start_end = merge_listinfo(self.start_end)
        print(outputs.shape, labels.shape, channel_name.shape, start_end.shape)
        res = {"outputs":outputs, 
                "labels":labels, 
                "channel_name":channel_name, 
                "start_end":start_end, 
...
class ClassificationStats(InferenceStats):
    ...
    def export_cvs(self, fn):
        ...
        result = np.concatenate(
            (
                channel_names.reshape(-1, 1),
                start_end,
                prediction_a.reshape(-1, 1),
                prediction_s.reshape(-1, 1),
                prediction_e.reshape(-1, 1),
                outputs_a.reshape(-1, 1),
                outputs_s.reshape(-1, 1),
                outputs_e.reshape(-1, 1),
            ),
            1,
        )
