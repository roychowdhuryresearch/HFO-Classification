...
%data_dir = '/mnt/SSD4/qiujing_data/New_EEG_data'
data_dir = '/mnt/SSD4/qiujing_data/EEG_data'
data_folder = dir(data_dir)
data_folder = {data_folder(1:end).name}
%data_folder = [{'Pt12_NB'}    {'Pt13_HJ'}    {'Pt14_NS'}   ]
%data_folder = [{'Pt16_AE'}    {'Pt17_SM'}    {'Pt18_MS'} ]%%%TODO AE B version. killed
%data_folder = [{'Pt17_SM'}    {'Pt18_MS'} ]
%data_folder = [{'Pt16_AE'} ]
data_folder = [ {'Pt2_AG'}]
%data_folder = [{'Pt20_CB'}    {'Pt21_TD'}    {'Pt2_AG'}    {'Pt3_KC'}    {'Pt4_HS'} ]
%data_folder = [ {'Pt5_DL'}   {'Pt6_MG'}    {'Pt7_GM'}    {'Pt8_ST'}    {'Pt9_NM'}]
for p = 1:numel(data_folder)
    patient_name = data_folder(p)
    
    patient_name= patient_name{1}
    if strcmp(patient_name, 'Pt1_AR')
        continue
    end
...
%     end
    ...
    %foler_name = fullfile(data_dir, patient_name)
    for j=1:2
        %file_name = [pure_name '2_'  bank_name(j)  '_ave_hfo.rhfe' ]
...
function f_LoadAnalysis(st_EvControl,st_EvInfo, st_Load, st_LoadInfo)
        ...
        %half_save_num = fix(number_electrodes/2)
        %half_save_num = 1
        %number_electrodes = 2
        
%         all_result0.channel_names = st_LoadInfo.v_ElecNames(1:half_save_num)   
%         all_result1.channel_names = st_LoadInfo.v_ElecNames(half_save_num+1:end) 
%         for i = 1: half_save_num
%              all_events{i} = f_LoadElectrode(st_EvControl,st_EvInfo, st_Load, st_LoadInfo, i);
%         end 
%         all_result0.all_events = all_events
%        
        
%         str_name	= [st_LoadInfo.str_PathName '/' st_LoadInfo.str_FileName(1:end-5) '_hfo_spike_info_500_0.mat'];
%         save(str_name, 'all_result0', '-v7.3');
%         clear all_result0 all_events
%         
%         for i = 1: (number_electrodes - half_save_num)
%              all_events{i} = f_LoadElectrode(st_EvControl,st_EvInfo, st_Load, st_LoadInfo, i + half_save_num);
%         end 
%         
%         all_result1.all_events = all_events
%         
%         str_name	= [st_LoadInfo.str_PathName '/' st_LoadInfo.str_FileName(1:end-5) '_hfo_spike_info_500_1.mat'];
%         save(str_name, 'all_result1', '-v7.3');
%         clear all_result1 all_events
%         
...
%         all_hfos_info = cell(number_electrodes,1);
        ...
        str_name	= [st_LoadInfo.str_PathName '/' st_LoadInfo.str_FileName(1:end-5) '_hfo_spike_info_500_10.mat'];
...
%         tmp_struct = [channel_HFO_info{1:end}];
%         %rm_fields = {'v_FreqLimits', 's_EvMaxFreq','s_CurMaxFreq' };
%         rm_fields = {'s_EvMaxFreq','s_CurMaxFreq' };
%         channel_HFO_info = rmfield(tmp_struct,rm_fields);
        % Draw Lines
