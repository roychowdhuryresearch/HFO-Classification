...
% Set HFO parameters
run('HFO_parameters_detector_mni')
st_FilePath = struct;
...
for n = 1 : number_patient
    ...
    rhfe_str_SaveFileName            = strcat(st_FilePath.name(1:end-4),'_mni_hfo.rhfe');
...
function f_Ini(s_Indicator,pst_Path)
        ...
        if s_Indicator
            ...
            clc;clear;close all
            %     warning off all
            st_Path.path    = which('run_MNI_detector');
...
%     str_TempFile        = [];
    ...
    str_TempFile = [str_TempFolder  str_TmpFName '~mni-tmp.mat'];
    load(str_TempFile)
...
function [st_ChInfo] = f_FileRead_modified(st_FilePath)
    ...
    v_CommonNel = vertcat(v_CommonNel,m_CellStr{3,kk});  
